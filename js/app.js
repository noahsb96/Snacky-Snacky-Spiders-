...
function startGame() { // start game function
    setInterval(() => { // every second
        bugArray.push(new Bug(), new Bug()); // 2 new bugs are pushed into the bug array
    }, 1000);    
    setInterval(() => { // every second
...
function bugMovement () { // bug movement function
    ...
    bugArray.forEach((bug, index) => { // for each bug in the bug array and it's corresponding index
        if (players.firstPlayer.xpos + players.firstPlayer.width >= bug.xpos && players.firstPlayer.xpos <= bug.xpos + bug.width && players.firstPlayer.ypos <= bug.ypos + bug.height && players.firstPlayer.ypos >= bug.ypos) { // if the first player's x position plus the first player's width is greater than or equal to bug's x position and the first player's x position is less than or equal to the bug's x position plus the bug's width and the first player's y position is less than or equal to the bug's y position plus the bug's height and the first player's y position is less than or equal to the bug's y position
        ...
        if (players.secondPlayer.xpos + players.secondPlayer.width >= bug.xpos && players.secondPlayer.xpos <= bug.xpos + bug.width && players.secondPlayer.ypos <= bug.ypos + bug.height && players.secondPlayer.ypos >= bug.ypos) { // if the second player's x position plus the second player's width is greater than or equal to bug's x position and the second player's x position is less than or equal to the bug's x position plus the bug's width and the second player's y position is less than or equal to the bug's y position plus the bug's height and the second player's y position is less than or equal to the bug's y position
                bugArray.splice(index, 1); // remove that bug from the bug array
